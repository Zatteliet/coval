...
def main():
    ...
    if keep_non_referring and keep_singletons:
        msg = (
            "all annotated markables, i.e. including corferent markables, "
            "singletons and non-referring markables"
        )
    elif keep_non_referring and not keep_singletons:
        msg = (
            "only coreferent markables and non-referring markables, "
            "excluding singletons"
        )
    elif not keep_non_referring and keep_singletons:
        msg = (
            "coreferring markables and singletons, " "excluding non-referring mentions."
        )
    else:
        msg = (
            "only coreferring markables, "
            "excluding singletons and non-referring mentions"
        )

    print(
        "The scorer is evaluating ",
        msg,
...
def evaluate(
    key_directory, sys_directory, metrics, keep_singletons, keep_non_referring, use_MIN
):
    doc_coref_infos, doc_non_referring_infos = reader.get_coref_infos(
        key_directory, sys_directory, keep_singletons, keep_non_referring, use_MIN
    )
