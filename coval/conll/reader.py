...
def get_doc_mentions(
    doc_name, doc_lines, keep_singletons, print_debug=False, word_column=3
):
    clusters = {}
    open_mentions = {}
    singletons_num = 0
    ...
    for sent_num, sent_line in enumerate(doc_lines):
        ...
        for word_index, line in enumerate(sent_line):
            sent_words.append(
                line.split()[word_column] if len(line.split()) > word_column + 1 else ""
            )

            single_token_coref, open_corefs, end_corefs = extract_coref_annotation(line)

            if single_token_coref:
                m = mention.Mention(
                    doc_name, sent_num, word_index, word_index, [sent_words[word_index]]
                )
...
def mask_unseen_mentions(clusters, seen_mentions, keep_singletons):
    ...
    for i in unseens:
        ...
        if len(clusters[i]) == 0 or (len(clusters[i]) == 1 and not keep_singletons):
...
def extract_coref_annotation(line):
    ...
    if len(single_token_coref) > 1:
        print(
            "Warning: A single mention is assigned to more than one cluster: %s"
            % single_token_coref
        )
...
def extract_annotated_parse(
    mention_lines, start_index, parse_column=5, word_column=3, POS_column=4
):
    ...
    for i, line in enumerate(mention_lines):
        ...
        for j, c in enumerate(parse):
            if c == "(":
                if tag_started:
                    node = mention.TreeNode(
                        "".join(tag_name), pos_tags, start_index + i, False
                    )
                    if open_nodes:
                ...
                if terminal_nodes:
                    ...
                    if any(c.isalpha() for c in terminal_nodes) or any(
                        c.isdigit() for c in terminal_nodes
                    ):
                        node = mention.TreeNode(
                            " ".join(terminal_nodes), pos_tags, start_index + i, True
                        )
                        if open_nodes:
                            if open_nodes[-1].children:
                                open_nodes[-1].children.append(node)
                ...
                node = mention.TreeNode("".join(tag_name), None, start_index + i, False)
...
def remove_nested_coref_mentions(clusters, keep_singletons, print_debug=False):
    ...
    for c_index in to_be_removed_mentions:
        ...
        if (
            len(clusters[c_index]) != 1
            and len(clusters[c_index]) - len(to_be_removed_mentions[c_index]) == 1
        ):
            ...
            clusters[c_index] = [
                m for m in clusters[c_index] if m not in to_be_removed_mentions[c_index]
            ]
...
def get_coref_infos(
    ...
    for doc in key_doc_lines:
        ...
        if NP_only or min_span:
            key_clusters = set_annotated_parse_trees(
        ...
        if NP_only or min_span:
            sys_clusters = set_annotated_parse_trees(
