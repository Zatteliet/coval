...
class Mention:
    ...
    def __str__(self):
        return str(
            ...
            + str(self.end)
            + ")"
            + (str(self.gold_parse) if self.gold_parse else "")
            + " "
            + " ".join(self.words)
        )
    ...
    def get_min_span_no_valid_tag(self, root):
        ...
        while queue:
            ...
            if node.isTerminal and depth <= terminal_shortest_depth:
                ...
                (not self.min_spans or depth < terminal_shortest_depth)
                and node.children
                and (depth == 0 or not accepted_tags or node.tag[0:2] in accepted_tags)
            ):
                for child in node.children:
                    if not child.isTerminal or (
                        accepted_tags and node.tag[0:2] in accepted_tags
                    ):
    ...
    def get_valid_node_min_span(self, root, valid_tags, min_spans):
        ...
        while queue:
            ...
            if node.isTerminal and depth <= terminal_shortest_depth:
                ...
                (not min_spans or depth < terminal_shortest_depth)
                and node.children
                and (depth == 0 or not valid_tags or node.tag[0:2] in valid_tags)
            ):
                for child in node.children:
                    if not child.isTerminal or (
                        valid_tags and node.tag[0:2] in valid_tags
                    ):
                        queue.append((child, depth + 1))
    ...
    def get_top_level_phrases(self, root, valid_tags):
        ...
        if (
            ...
            for node in root.children:
                if node:
                    if node.isTerminal and self.is_a_valid_terminal_node(
                        node.tag, node.pos
                    ):
                        self.min_spans.add((node.tag, node.index))
    ...
    def get_valid_tags(self, root):
        ...
        if root.tag[0:2] == "VP":
            ...
            if (
                root.children
            ):  ## If none of the first level nodes are either NP or VP, examines their children for valid mention tags
                all_tags = []
                for node in root.children:
