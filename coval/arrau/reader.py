...
def get_doc_markables(
    ...
    for word_index, line in enumerate(doc_lines):
        ...
        if len(columns) > 1:
            ...
            MIN_annotations = (
                columns[MIN_column].split("@")
                if extract_MIN and len(columns) >= 4
                else None
            )
            ...
            if print_debug:
                if (
                    MIN_annotations
                    and len(markable_annotations) != len(MIN_annotations)
                ) or (
                    coref_annotations
                    and len(markable_annotations) != len(coref_annotations)
                ):
                    print(
                        (
            ...
            for i, markable_annotation in enumerate(markable_annotations):
                markable_id = int(
                    markable_annotation[11 : markable_annotation.find("=")]
                )
                cluster_id = int(
                    markable_annotation[markable_annotation.find("=") + 5 :]
                )
                ...
                if markable_annotation.startswith("B-markable_"):
                    ...
                    if (
                        coref_annotations
                        and len(markable_annotations) == len(coref_annotations)
                        and coref_annotations[i].strip() == "non_referring"
                    ):
...
def get_coref_infos(
    ...
    for doc in key_docs:
        ...
        (
            key_clusters,
            key_non_referrings,
            key_removed_non_referring,
            key_removed_singletons,
        ) = process_clusters(key_clusters, keep_singletons, keep_non_referring)
        (
            sys_clusters,
            sys_non_referrings,
            sys_removed_non_referring,
            sys_removed_singletons,
        ) = process_clusters(sys_clusters, keep_singletons, keep_non_referring)
        ...
        if print_debug and not keep_non_referring:
            print(
                "%s and %s non-referring markables are removed from the "
        ...
        if print_debug and not keep_singletons:
            print(
                "%s and %s singletons are removed from the evaluations of "
