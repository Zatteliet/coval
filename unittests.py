...
def read(key, response):
    return get_coref_infos("tests/%s" % key, "tests/%s" % response, False, False, True)
...
def test_A3():
    ...
    assert evaluate(doc, b_cubed) == approx([6 / 6, (4 + 7 / 12) / 9, 110 / 163])
...
def test_A11():
    ...
    assert evaluate(doc, lea) == approx(
        [(0 + 2 + 3) / 6, 4 / 15, 2 * 5 / 6 * 4 / 15 / (5 / 6 + 4 / 15)]
    )
...
def test_E1():
    ...
    assert evaluate(doc, lea) == approx(
        [
            (5 + 2 + 5) / 12,
            (10 * (20 / 45) + 2) / 12,
            2 * 1 * ((10 * (20 / 45) + 2) / 12) / (1 + ((10 * (20 / 45) + 2) / 12)),
        ]
    )
...
def test_J1():
    ...
    assert evaluate(doc, lea) == approx([(3 * 1 / 3) / 3, 1, 2 * 1 / 3 / (1 + 1 / 3)])
